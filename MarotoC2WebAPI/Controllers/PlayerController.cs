...
namespace MarotoC2WebAPI.Controllers
{
    [Authorize]
    ...
    public class PlayerController : ApiController
    {
        ...
        public IHttpActionResult RegisterPlayer(PlayerModel player)
        {
            ...
            {
                string sql = "INSERT INTO dbo.Player(Id, Nickname, Date) " +
                    $"VALUES('{player.Id}','{player.Nickname}','{player.Date}')";
                try
                {
                    cnn.Execute(sql);
                }
        ...
        public PlayerModel GetPlayerInfo()
        {
            ...
            using (IDbConnection cnn = new ApplicationDbContext().Database.Connection)
            {
                string sql = $"SELECT Id, Nickname, Date FROM dbo.Player " +
